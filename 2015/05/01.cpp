...
bool isNiceString(string s) {
  ...
  vector<string> forbidden_words = {"ab", "cd", "pq", "xy"};

  int num_vowels = 0;
  bool num_consective = false;
  bool contains_forbidden_word = false;
  ...
  for (string forbidden_word : forbidden_words) {
    if (s.find(forbidden_word) != string::npos) {
      contains_forbidden_word = true;
    }
  }

  return (num_vowels >= 3) && (num_consective) && (!contains_forbidden_word);
}
