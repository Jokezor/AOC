...
void solution() {
  ...
  // We can either simply append more copies of the cards.
  // But we can compute it in another way.
  // We can keep a counter of how many of those cards we have.
  // We can keep a vector since we have n cards and
  // those we will be processed in order.
  vector<int> card_count(card_numbers.size(), 1);
  ...
  for (int i = 0; i < winning_numbers.size(); i++) {
    ...
    // Add in for the cards extra.
    for (int c = 1; c <= card_points; c++) {
      card_count[i + c] += card_count[i];
    }
    ans += card_count[i];
  }
