...
void solution() {
  ...
  for (ll direction : directions) {
    ...
    // 10 moves left in the direction, 0 heat loss
    // Need to keep track
    q.push({0, 0, direction, 10, 0LL});
    visited.insert({0, 0, direction, 10});

    while (!q.empty()) {
      tuple<ll, ll, ll, ll, ll> curr = q.front();
      q.pop();

      ll x = get<0>(curr);
      ll y = get<1>(curr);
      ll direction = get<2>(curr);
      ll direction_moves = get<3>(curr);
      ll curr_length = get<4>(curr);

      // cout << x << ": " << y << "\n";

      vector<int> possible_directions;
      // Now we can turn.
      if (direction_moves <= 7) {
        for (ll possible_direction : direction_map[direction]) {
          possible_directions.push_back(possible_direction);
        }
      } else {
      ...
      for (ll possible_direction : possible_directions) {
        ll possible_direction_moves = 10;
        ...
        tuple<ll, ll, ll, ll, ll> pos = {
            x + walk.first, y + walk.second, possible_direction,
            possible_direction_moves,
            curr_length + dp[x + walk.first][y + walk.second]};

        if (possible_direction_moves > 0LL) {
          tuple<ll, ll, ll, ll> pos_map = {get<0>(pos), get<1>(pos),
                                           get<2>(pos), get<3>(pos)};
          if (positions[pos_map] == 0LL || positions[pos_map] > get<4>(pos)) {
            q.push(pos);
            positions[pos_map] = (ll)get<4>(pos);
            // cout << get<4>(pos) << "\n";
          }
        }
      }
  ...
  for (int direction : directions) {
    // Here we match the end condigition of 4 in a row.
    for (int i = 0; i <= 7; i++) {
