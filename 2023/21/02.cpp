...
int get_rolling_index(int x, int max) {
  int new_index;
...
void solution() {
  ...
  while (!q.empty()) {
    ...
    for (pair<int, int> dir : directions) {
      // Cyclic, 0 -1 = m-1
      // n-1+1 = 0
      // n-1+1 solved by node.first%n
      // 0-1 solved by m-1 - node.second%m
      // node.second%m
      pair<int, int> node = {(coord.first + dir.first),
                             (coord.second + dir.second)};
      if (rows[node.first % rows.size()][node.second % rows[0].length()] ==
              '.' &&
          visited.find(node) == visited.end()) {
        if (node_parity[node] == 0 || node_parity[node] % 2) {
          q.push({node, steps + 1});
          visited.insert(node);
        }
      }
