...
ll part_1(vector<string> rows) {
    ...
    // count all '^' that have a beam hit it.
    // It will if it has a '^' to the right or left of it.
    // which has a higher index than any straight on top of it.
    // But that is only if they in turn have a beam on them etc.
    // KISS is:
    // walk down from S, adding '|'.
