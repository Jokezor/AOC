...
ll part_1(vector<string> rows) {
    // Max from each bank is simply the first largest digit from the left followed by the largest digit of the right of it.
    ...
    for (string row : rows) {
        ...
        for (int i=0; i < n - 1; ++i) {
            if (row[i] > batteries[0]) {
                batteries[0] = row[i];
                max_ind = i;
            }
        }

        // Now go from max_ind to n.
        for (int i=max_ind+1; i < n; ++i) {
            if (row[i] > batteries[1]) {
                batteries[1] = row[i];
            }
        }
