...
pair<ll, ll> get_range_pair(string row) {
    // split on 
    vector<string> split = split_by(row, '-');
    assert(split.size() == 2);
    pair<ll, ll> range_pair = {stoll(split[0]), stoll(split[1])};
    return range_pair;
}
...
ll part_1(vector<string> rows) {
    ...
    for (string row : rows) {
        ...
        if (!is_ingredients) {
            // parse out left and right
            pair<ll, ll> range_pair = get_range_pair(row);
            ranges.push_back(range_pair);
        }
        else {
            ingredients.push_back(stoll(row));
        }
    }
...
ll part_2(vector<string> rows) {
    ...
    // We Probably need to binary search!
