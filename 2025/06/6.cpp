...
ll part_1(vector<string> rows) {
    ...
    // Get the width of the numbers by checking how many '*' or '+'
    // there are.
    //
    // Then we simply go in order, add each integer.
    // Also check on what vertical each '*' or '+' are.
    // Assert that they are at the same "depth".
    vector<char> operations;

    for (char c : rows[rows.size()-1]) {
        if (c == '*' || c == '+') {
    ...
    vector<vector<ll>> numbers(m, vector<ll>(n));


    for (int i=0; i < n; ++i) {
        string row = rows[i];
        auto string_nums = split_by(row, ' ');
        int j = 0;

        for (string num : string_nums) {
            num.erase(remove(num.begin(), num.end(), ' '), num.end());
            if (num != "") {
                numbers[j][i] = stoll(num);
                ++j;
            }
        }
        assert(j == m);
    ...
    for (int i=0; i < m; ++i) {
        vector<ll> sub_problem = numbers[i];
        char operation = operations[i];

        ll start = sub_problem[0];

        for (int i=1; i < n; ++i) {
            if (operation == '*') start *= sub_problem[i];
            if (operation == '+') start += sub_problem[i];
        }
        ans += start;
    }
...
unsigned ll part_2(vector<string> rows) {
    ...
    // Now the number of numbers are the width of each separate number.
    // Not known in advance.
    vector<vector<unsigned ll>> numbers(m);

    // Now go through each column in turn until i==n-1
    int k = 0;
    ...
    for (int j=0; j < rows[0].length(); ++j) {
        ...
        if (num == "") {
            ...
            assert(k <= m);
            numbers[k].push_back(stoull(num));
        }
    }

    for (int i=0; i < m; ++i) {
        vector<unsigned ll> sub_problem = numbers[i];
        char operation = operations[i];

        unsigned ll start = sub_problem[0];

        for (int i=1; i < sub_problem.size(); ++i) {
            if (operation == '*') start *= sub_problem[i];
            if (operation == '+') start += sub_problem[i];
        }
        ans += start;
